...
description: "Review local LinkSmith refs when the task is a PR review, branch review, or change review for this repository and the reviewer should apply LinkSmith's contract-first, deterministic-first standards."
---
...
1. Run `scripts/review-branch.ps1` when local source and target refs are available.
2. Read:
...
- The default review path is local ref vs local ref in this repo.
- Prefer named branches or tags when available, but commit hashes are acceptable review baselines.
- `SourceBranch` and `TargetBranch` remain valid parameter aliases for backward compatibility.
